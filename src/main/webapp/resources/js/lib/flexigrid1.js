	...
	$.addFlex = function(t,p)
	{
		...
		var g = {
			...
			populate: function () { //get latest data
				...
				///修改，因为需要配合struts2处理，配合appframe中的com.ai.util.page对象使用
				//，需要在这里加上pageModel的属性；
				var param = [{ 
				    name : p.pageModel ? p.pageModel + '.pageNo' : 'pageNo', 
				    value : p.newp 
				  },{
				  	name : p.pageModel ? p.pageModel + '.pageSize' : 'pageSize',
				  	value : p.rp 
				  },{
				  	name : p.pageModel ? p.pageModel +'.order':'order',
				  	value : p.sortname
				  },{
				  	name : p.pageModel ? p.pageModel +'.orderBy':'orderBy',
				  	value : p.sortorder 
				  }/*,{
				  	name : p.pageModel ? p.pageModel +'.query':'query',
				  	value : p.query
				  },{
				  	name : p.pageModel ? p.pageModel +'.qtype':'qtype',
				  	value : p.qtype
				  }*/
				];

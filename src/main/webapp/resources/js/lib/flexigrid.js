				...
				if (p.dataType == 'json') {
					$.each(data.rows, function(i, row) {
						    ...
						    $('thead tr:first th', g.hDiv).each(function() {
									...
									if (p.colModel[idx].showTdTitle !== false) {
										td.setAttribute("title", tdVal[idx]);
									}
...
//				}
				...
				$.ajax({
					    type : p.method,
					    url : p.url,
					    data : p.ajax_getData(), // param
					    dataType : p.dataType,
					    contentType : p.ajax_contentType,
		...
		if (p.colModel) {
			...
			for (var i = 0; i < p.colModel.length; i++) {
				...
				if (cm.showThTitle !== false) {
					$(th).attr("title", cm.display);
				}
